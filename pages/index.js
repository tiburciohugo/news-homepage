...
export default function Home() {
  return (
    <div className="lg:flex lg:flex-col lg:justify-center lg:items-center">
          ...
          <aside className="bg-very-dark-blue mt-12 h-full c-md:w-4/12 flex flex-col py-4 px-6 md:mt-14">
            ...
            {posts ? (
              <div>
                {posts.map((post) => (
                  <div
                    className="py-4 border-t-[0.063rem] border-dark-grayish-blue first:border-none"
                    key={post.id}
                  >
                    <Link href={`/posts/${post.id}`}>
                      <h4 className="text-white text-lg leading-4 font-bold mt-4 hover:text-soft-orange">
                        {post.title}
                      </h4>
                    </Link>
                    <p className="text-grayish-blue text-sm font-semibold my-2">
                      {post.content}
                    </p>
                  </div>
                ))}
              ...
              {articles.map((article) => {
                return (
                  ...
                  >
                    ...
                    <div
                      key={article.id}
                      className="flex flex-col gap-y-2  md:gap-0 lg:gap-y-2 md:w-4/6 lg:w-full h-full px-2 md:px-0 md:ml-4"
                    >
                      ...
                      <p className="text-dark-grayish-blue text-sm font-semibold xl:w-[80%]">
                        {article.content}
                      </p>
                    </div>
                  </div>
      ...
      <footer className="flex justify-center items-center gap-4 mt-8 p-2 w-full max-w-6xl max-h-14 border-t bottom-0">
        <a
